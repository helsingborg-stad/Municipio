...
@openStreetMap([
        ...
        'fullWidth' => true,
        'expanded' => true,
        'startPosition' => array('lat' => $post->location['lat'], 'lng' => $post->location['lng'], 'zoom' => 17)
    ])
@slot('sidebarContent')
@if (!empty($featuredImage['src']))
    ...
    <div class="o-container">
        {{-- @php echo '<pre>' . print_r( $post->location, true ) . '</pre>';@endphp --}}
        @paper([
            'attributeList' => [
                'style' => !empty($featuredImage['src']) ? 'transform:translateY(calc(max(-50%, -50px)))' : 'margin-top: 32px',
                'data-js-map-location' => json_encode($post->location['pin']),
