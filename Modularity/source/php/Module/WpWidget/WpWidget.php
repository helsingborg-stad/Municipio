...
class WpWidget extends \Modularity\Module
{
    ...
    public function init()
    {
        ...
        add_filter('acf/load_field/name=mod_standard_widget_type', [$this, 'addWidgetOptionsList']);
        add_filter('acf/load_field/name=wp_widget_tag_cloud_taxonomy', [$this, 'tagCloudTaxonomies']);
    }

    public function data(): array
    {
        $data = $this->getFields();
        $data['settings'] = \Modularity\Module\WpWidget\WpWidget::createSettingsArray(
            $data['mod_standard_widget_type'],
            $this->ID,
        );
        $data['widgetBefore'] = apply_filters(
            'Modularity/Module/WpWidget/before',
            '<div class="box">',
            $this->args,
            $this,
        );
     ...
     * @return void
     */
    public static function displayWidget($widget, $instance = [])
    {
        if (
            array_key_exists($widget, \Modularity\Module\WpWidget\WpWidget::getWidgetIndexList())
            && class_exists($widget)
        ) {
