function loadMoreHandler(event) {
	...
	const parent = event.target.closest(".mod-curator-load-more"),
		embedCode = parent.getAttribute("data-code"),
		itemCount = parseInt(parent.getAttribute("data-item-count")),
		itemsPerPage = parseInt(parent.getAttribute("data-items-per-page"));
	...
	const grandParent = parent.closest(".modularity-socialmedia-container"),
		socialMediaBlocks = grandParent.querySelector(
			".modularity-socialmedia__content",
		),
		placeholderBlocks = socialMediaBlocks.querySelectorAll(
...
function updateItems(posts, button) {
	...
	const itemsLoaded = parseInt(button.getAttribute("data-items-loaded")),
		newItemsLoaded = itemsLoaded + posts.length;
...
function renderPosts(posts, button) {
	...
	const closestParent = button.closest(".modularity-socialmedia-container"),
		socialMediaBlocks = closestParent.querySelector(
			".modularity-socialmedia__content",
		),
		placeholderBlock = socialMediaBlocks.querySelector(
			".modularity-socialmedia__item--placeholder",
		),
		placeholderBlocks = socialMediaBlocks.querySelectorAll(
			".modularity-socialmedia__item--placeholder",
		),
		columnClasses = socialMediaBlocks
