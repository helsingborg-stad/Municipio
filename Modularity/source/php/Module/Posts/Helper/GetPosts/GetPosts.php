...
class GetPosts implements GetPostsInterface
{
    ...
    public function getPosts(): PostsResultInterface
    {
        ...
        $stickyPosts = $this->sortPosts(
            $stickyPosts,
            $this->fields['posts_sort_by'] ?? 'date',
            $this->fields['posts_sort_order'] ?? 'desc',
        );
        $posts = $wpQuery->get_posts();

        return $this->formatResponse($posts, $wpQuery->max_num_pages, $stickyPosts);
    }
    ...
    private function getPostArgs(array $fields, int $page, array $stickyPostIds = [])
    {
        ...
        switch ($fields['posts_data_source'] ?? []) {
            ...
            case 'schematype':
                ...
                $getPostsArgs['post_type'] = $this->postTypesFromSchemaTypeResolver->resolve(
                    $fields['posts_data_schema_type'],
                );
                break;
        }
