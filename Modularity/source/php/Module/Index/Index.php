...
class Index extends \Modularity\Module
{
    ...
    public function init()
    {
        ...
        add_filter('acf/fields/post_object/query/key=field_569cf1252cfc9', [$this, 'postObjectQuery'], 10, 3);
    }
            ...
            foreach ($items as $key => &$item) {
                ...
                if ($postData !== false && get_post_status($postData->ID)) {
                    ...
                    if (is_object($postData) && isset($postData->ID)) {
                        ...
                        $item['lead'] = $this->switchContent(
                            $item['lead'],
                            $this->parseExcerpt($postData->post_content),
                        );
                    }
                } else {
                    unset($item);
    ...
    public function getImageDimension($columnClass)
    {
        switch ($columnClass) {
            case 'o-grid-12@md': // 1-col
                $this->thumbnailSize = [1200, 900];
                return $this->thumbnailSize;

            case 'o-grid-6@md': // 2-col
                $this->thumbnailSize = [800, 600];
                return $this->thumbnailSize;

            default:
                $this->thumbnailSize = [400, 300];
                return $this->thumbnailSize;
        }
    }
