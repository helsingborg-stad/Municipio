<?php

if (function_exists('acf_add_local_field_group')) {
    acf_add_local_field_group(array(
    'key'                   => 'group_5805e5dc0a3be',
    'title'                 => __('Contacts', 'municipio'),
    'fields'                => array(
        0 => array(
            'key'                => 'field_5805e5dc1ddcd',
            'label'              => __('Columns', 'municipio'),
            'name'               => 'columns',
            'aria-label'         => '',
            'type'               => 'select',
            'instructions'       => '',
            'required'           => 1,
            'conditional_logic'  => 0,
            'wrapper'            => array(
                'width' => '',
                'class' => '',
                'id'    => '',
            ),
            'choices'            => array(
                'grid-md-12' => __('1', 'municipio'),
                'grid-md-6'  => __('2', 'municipio'),
                'grid-md-4'  => __('3', 'municipio'),
                'grid-md-3'  => __('4', 'municipio'),
            ),
            'default_value'      => 'o-grid-12@md',
            'return_format'      => 'value',
            'multiple'           => 0,
            'allow_null'         => 0,
            'ui'                 => 0,
            'ajax'               => 0,
            'placeholder'        => '',
            'allow_custom'       => 0,
            'search_placeholder' => '',
        ),
        1 => array(
            'key'               => 'field_6851b66acc73f',
            'label'             => __('View', 'municipio'),
            'name'              => 'view',
            'aria-label'        => '',
            'type'              => 'button_group',
            'instructions'      => '',
            'required'          => 0,
            'conditional_logic' => 0,
            'wrapper'           => array(
                'width' => '',
                'class' => '',
                'id'    => '',
            ),
            'choices'           => array(
                'simple'   => __('Simple', 'municipio'),
                'extended' => __('Extended', 'municipio'),
            ),
            'default_value'     => 'extended',
            'return_format'     => 'value',
            'allow_null'        => 0,
            'layout'            => 'horizontal',
        ),
        2 => array(
            'key'               => 'field_6724b0113e1b6',
            'label'             => __('Placeholder avatar', 'municipio'),
            'name'              => 'placeholder_avatar',
            'aria-label'        => '',
            'type'              => 'true_false',
            'instructions'      => __('If the items that are missing an avatar image should show a placeholder avatar. This avatar will be based on the name of the author.', 'municipio'),
            'required'          => 0,
            'conditional_logic' => 0,
            'wrapper'           => array(
                'width' => '',
                'class' => '',
                'id'    => '',
            ),
            'message'           => '',
            'default_value'     => 1,
            'ui_on_text'        => '',
            'ui_off_text'       => '',
            'ui'                => 1,
        ),
        3 => array(
            'key'                               => 'field_5805e5dc1dc55',
            'label'                             => __('Contacts', 'municipio'),
            'name'                              => 'contacts',
            'aria-label'                        => '',
            'type'                              => 'flexible_content',
            'instructions'                      => '',
            'required'                          => 1,
            'conditional_logic'                 => 0,
            'wrapper'                           => array(
                'width' => '',
                'class' => '',
                'id'    => '',
            ),
            'acfe_flexible_advanced'            => 0,
            'layouts'                           => array(
                '5757b95767730' => array(
                    'key'                           => '5757b95767730',
                    'name'                          => 'custom',
                    'label'                         => __('Custom', 'municipio'),
                    'display'                       => 'block',
                    'sub_fields'                    => array(
                        0  => array(
                            'key'               => 'field_5805e5dc26dde',
                            'label'             => __('Image', 'municipio'),
                            'name'              => 'image',
                            'aria-label'        => '',
                            'type'              => 'image',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'uploader'          => '',
                            'return_format'     => 'array',
                            'acfe_thumbnail'    => 0,
                            'min_width'         => '',
                            'min_height'        => '',
                            'min_size'          => '',
                            'max_width'         => '',
                            'max_height'        => '',
                            'max_size'          => '',
                            'mime_types'        => '',
                            'preview_size'      => 'thumbnail',
                            'library'           => 'all',
                        ),
                        1  => array(
                            'key'               => 'field_5805e5dc27255',
                            'label'             => __('First name', 'municipio'),
                            'name'              => 'first_name',
                            'aria-label'        => '',
                            'type'              => 'text',
                            'instructions'      => '',
                            'required'          => 1,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'maxlength'         => '',
                            'placeholder'       => '',
                            'prepend'           => '',
                            'append'            => '',
                        ),
                        2  => array(
                            'key'               => 'field_5805e5dc276e1',
                            'label'             => __('Surname', 'municipio'),
                            'name'              => 'last_name',
                            'aria-label'        => '',
                            'type'              => 'text',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'maxlength'         => '',
                            'placeholder'       => '',
                            'prepend'           => '',
                            'append'            => '',
                        ),
                        3  => array(
                            'key'               => 'field_5805e5dc2771c',
                            'label'             => __('Job title', 'municipio'),
                            'name'              => 'work_title',
                            'aria-label'        => '',
                            'type'              => 'text',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'maxlength'         => '',
                            'placeholder'       => '',
                            'prepend'           => '',
                            'append'            => '',
                        ),
                        4  => array(
                            'key'               => 'field_5805e5dc277e3',
                            'label'             => __('Administration unit', 'municipio'),
                            'name'              => 'administration_unit',
                            'aria-label'        => '',
                            'type'              => 'text',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'maxlength'         => '',
                            'placeholder'       => '',
                            'prepend'           => '',
                            'append'            => '',
                        ),
                        5  => array(
                            'key'               => 'field_5805e5dc27b58',
                            'label'             => __('E-mail', 'municipio'),
                            'name'              => 'email',
                            'aria-label'        => '',
                            'type'              => 'email',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => 0,
                            'wrapper'           => array(
                                'width' => '100',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'placeholder'       => '',
                            'prepend'           => '',
                            'append'            => '',
                        ),
                        6  => array(
                            'key'                           => 'field_5805e62f94d0f',
                            'label'                         => __('Phone numbers', 'municipio'),
                            'name'                          => 'phone_numbers',
                            'aria-label'                    => '',
                            'type'                          => 'repeater',
                            'instructions'                  => '',
                            'required'                      => 0,
                            'conditional_logic'             => 0,
                            'wrapper'                       => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'acfe_repeater_stylised_button' => 0,
                            'layout'                        => 'table',
                            'min'                           => 0,
                            'max'                           => 2,
                            'collapsed'                     => '',
                            'button_label'                  => __('Add phone number', 'municipio'),
                            'rows_per_page'                 => 20,
                            'sub_fields'                    => array(
                                0 => array(
                                    'key'                => 'field_5bf6aa828136b',
                                    'label'              => __('Type', 'municipio'),
                                    'name'               => 'type',
                                    'aria-label'         => '',
                                    'type'               => 'select',
                                    'instructions'       => '',
                                    'required'           => 0,
                                    'conditional_logic'  => 0,
                                    'wrapper'            => array(
                                        'width' => '20',
                                        'class' => '',
                                        'id'    => '',
                                    ),
                                    'choices'            => array(
                                        'phone'      => __('Landline', 'municipio'),
                                        'smartphone' => __('Cell phone', 'municipio'),
                                    ),
                                    'default_value'      => 'phone',
                                    'return_format'      => 'value',
                                    'multiple'           => 0,
                                    'allow_null'         => 0,
                                    'ui'                 => 0,
                                    'ajax'               => 0,
                                    'placeholder'        => '',
                                    'allow_custom'       => 0,
                                    'search_placeholder' => '',
                                    'parent_repeater'    => 'field_5805e62f94d0f',
                                ),
                                1 => array(
                                    'key'               => 'field_5805e64a94d10',
                                    'label'             => __('Phone number', 'municipio'),
                                    'name'              => 'number',
                                    'aria-label'        => '',
                                    'type'              => 'text',
                                    'instructions'      => '',
                                    'required'          => 0,
                                    'conditional_logic' => 0,
                                    'wrapper'           => array(
                                        'width' => '',
                                        'class' => '',
                                        'id'    => '',
                                    ),
                                    'default_value'     => '',
                                    'maxlength'         => '',
                                    'placeholder'       => '',
                                    'prepend'           => '',
                                    'append'            => '',
                                    'parent_repeater'   => 'field_5805e62f94d0f',
                                ),
                            ),
                        ),
                        7  => array(
                            'key'                           => 'field_5bf6a5fbc1b6b',
                            'label'                         => __('Social Media', 'municipio'),
                            'name'                          => 'social_media',
                            'aria-label'                    => '',
                            'type'                          => 'repeater',
                            'instructions'                  => '',
                            'required'                      => 0,
                            'conditional_logic'             => array(
                                0 => array(
                                    0 => array(
                                        'field'    => 'field_6851b66acc73f',
                                        'operator' => '==',
                                        'value'    => 'extended',
                                    ),
                                ),
                            ),
                            'wrapper'                       => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'acfe_repeater_stylised_button' => 0,
                            'layout'                        => 'table',
                            'min'                           => 0,
                            'max'                           => 0,
                            'collapsed'                     => '',
                            'button_label'                  => __('Lägg till rad', 'municipio'),
                            'rows_per_page'                 => 20,
                            'sub_fields'                    => array(
                                0 => array(
                                    'key'                => 'field_5bf6a737c1b6c',
                                    'label'              => __('Media', 'municipio'),
                                    'name'               => 'media',
                                    'aria-label'         => '',
                                    'type'               => 'select',
                                    'instructions'       => '',
                                    'required'           => 1,
                                    'conditional_logic'  => 0,
                                    'wrapper'            => array(
                                        'width' => '',
                                        'class' => '',
                                        'id'    => '',
                                    ),
                                    'choices'            => array(
                                        'facebook'  => __('Facebook', 'municipio'),
                                        'linkedin'  => __('LinkedIn', 'municipio'),
                                        'twitter'   => __('Twitter', 'municipio'),
                                        'instagram' => __('Instagram', 'municipio'),
                                    ),
                                    'default_value'      => 'facebook',
                                    'allow_null'         => 0,
                                    'multiple'           => 0,
                                    'ui'                 => 0,
                                    'return_format'      => 'value',
                                    'ajax'               => 0,
                                    'placeholder'        => '',
                                    'allow_custom'       => 0,
                                    'search_placeholder' => '',
                                    'parent_repeater'    => 'field_5bf6a5fbc1b6b',
                                ),
                                1 => array(
                                    'key'               => 'field_5bf6a991c1b6d',
                                    'label'             => __('URL', 'municipio'),
                                    'name'              => 'url',
                                    'aria-label'        => '',
                                    'type'              => 'url',
                                    'instructions'      => '',
                                    'required'          => 1,
                                    'conditional_logic' => 0,
                                    'wrapper'           => array(
                                        'width' => '',
                                        'class' => '',
                                        'id'    => '',
                                    ),
                                    'default_value'     => '',
                                    'placeholder'       => '',
                                    'parent_repeater'   => 'field_5bf6a5fbc1b6b',
                                ),
                            ),
                        ),
                        8  => array(
                            'key'                => 'field_5805e5dc28d3a',
                            'label'              => __('Address', 'municipio'),
                            'name'               => 'address',
                            'aria-label'         => '',
                            'type'               => 'textarea',
                            'instructions'       => '',
                            'required'           => 0,
                            'conditional_logic'  => 0,
                            'wrapper'            => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'      => '',
                            'acfe_textarea_code' => 0,
                            'maxlength'          => '',
                            'rows'               => '',
                            'placeholder'        => '',
                            'new_lines'          => 'wpautop',
                        ),
                        9  => array(
                            'key'                => 'field_5805e5dc28e30',
                            'label'              => __('Visiting address', 'municipio'),
                            'name'               => 'visiting_address',
                            'aria-label'         => '',
                            'type'               => 'textarea',
                            'instructions'       => '',
                            'required'           => 0,
                            'conditional_logic'  => array(
                                0 => array(
                                    0 => array(
                                        'field'    => 'field_6851b66acc73f',
                                        'operator' => '==',
                                        'value'    => 'extended',
                                    ),
                                ),
                            ),
                            'wrapper'            => array(
                                'width' => '50',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'      => '',
                            'acfe_textarea_code' => 0,
                            'maxlength'          => '',
                            'rows'               => '',
                            'placeholder'        => '',
                            'new_lines'          => 'wpautop',
                        ),
                        10 => array(
                            'key'                => 'field_5805e5dc29114',
                            'label'              => __('Opening hours', 'municipio'),
                            'name'               => 'opening_hours',
                            'aria-label'         => '',
                            'type'               => 'textarea',
                            'instructions'       => '',
                            'required'           => 0,
                            'conditional_logic'  => array(
                                0 => array(
                                    0 => array(
                                        'field'    => 'field_6851b66acc73f',
                                        'operator' => '==',
                                        'value'    => 'extended',
                                    ),
                                ),
                            ),
                            'wrapper'            => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'      => '',
                            'acfe_textarea_code' => 0,
                            'maxlength'          => '',
                            'rows'               => '',
                            'placeholder'        => '',
                            'new_lines'          => 'wpautop',
                        ),
                        11 => array(
                            'key'               => 'field_5805e5dc29182',
                            'label'             => __('Other', 'municipio'),
                            'name'              => 'other',
                            'aria-label'        => '',
                            'type'              => 'wysiwyg',
                            'instructions'      => '',
                            'required'          => 0,
                            'conditional_logic' => array(
                                0 => array(
                                    0 => array(
                                        'field'    => 'field_6851b66acc73f',
                                        'operator' => '==',
                                        'value'    => 'extended',
                                    ),
                                ),
                            ),
                            'wrapper'           => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'default_value'     => '',
                            'tabs'              => 'all',
                            'toolbar'           => 'full',
                            'media_upload'      => 1,
                            'delay'             => 0,
                        ),
                    ),
                    'min'                           => '',
                    'max'                           => '',
                    'acfe_flexible_render_template' => false,
                    'acfe_flexible_render_style'    => false,
                    'acfe_flexible_render_script'   => false,
                    'acfe_flexible_thumbnail'       => false,
                    'acfe_flexible_settings'        => false,
                    'acfe_flexible_settings_size'   => 'medium',
                    'acfe_flexible_modal_edit_size' => false,
                    'acfe_flexible_category'        => false,
                ),
                '5757b97ffecc6' => array(
                    'key'                           => '5757b97ffecc6',
                    'name'                          => 'user',
                    'label'                         => __('User', 'municipio'),
                    'display'                       => 'block',
                    'sub_fields'                    => array(
                        0 => array(
                            'key'                  => 'field_5805e5dc291c4',
                            'label'                => __('Select user', 'municipio'),
                            'name'                 => 'user',
                            'aria-label'           => '',
                            'type'                 => 'user',
                            'instructions'         => '',
                            'required'             => 1,
                            'conditional_logic'    => 0,
                            'wrapper'              => array(
                                'width' => '',
                                'class' => '',
                                'id'    => '',
                            ),
                            'role'                 => '',
                            'return_format'        => 'array',
                            'multiple'             => 0,
                            'acfe_bidirectional'   => array(
                                'acfe_bidirectional_enabled' => '0',
                            ),
                            'allow_null'           => 0,
                            'bidirectional'        => 0,
                            'bidirectional_target' => array(
                            ),
                        ),
                    ),
                    'min'                           => '',
                    'max'                           => '',
                    'acfe_flexible_render_template' => false,
                    'acfe_flexible_render_style'    => false,
                    'acfe_flexible_render_script'   => false,
                    'acfe_flexible_thumbnail'       => false,
                    'acfe_flexible_settings'        => false,
                    'acfe_flexible_settings_size'   => 'medium',
                    'acfe_flexible_modal_edit_size' => false,
                    'acfe_flexible_category'        => false,
                ),
            ),
            'min'                               => 1,
            'max'                               => '',
            'button_label'                      => __('Lägg till kontakt', 'municipio'),
            'acfe_flexible_stylised_button'     => false,
            'acfe_flexible_hide_empty_message'  => false,
            'acfe_flexible_empty_message'       => '',
            'acfe_flexible_layouts_templates'   => false,
            'acfe_flexible_layouts_previews'    => false,
            'acfe_flexible_layouts_placeholder' => false,
            'acfe_flexible_layouts_thumbnails'  => false,
            'acfe_flexible_layouts_settings'    => false,
            'acfe_flexible_async'               => array(
            ),
            'acfe_flexible_add_actions'         => array(
            ),
            'acfe_flexible_remove_button'       => array(
            ),
            'acfe_flexible_layouts_state'       => false,
            'acfe_flexible_modal_edit'          => array(
                'acfe_flexible_modal_edit_enabled' => false,
                'acfe_flexible_modal_edit_size'    => 'large',
            ),
            'acfe_flexible_modal'               => array(
                'acfe_flexible_modal_enabled'    => false,
                'acfe_flexible_modal_title'      => false,
                'acfe_flexible_modal_size'       => 'full',
                'acfe_flexible_modal_col'        => '4',
                'acfe_flexible_modal_categories' => false,
            ),
        ),
    ),
    'location'              => array(
        0 => array(
            0 => array(
                'param'    => 'post_type',
                'operator' => '==',
                'value'    => 'mod-contacts',
            ),
        ),
        1 => array(
            0 => array(
                'param'    => 'block',
                'operator' => '==',
                'value'    => 'acf/contacts',
            ),
        ),
    ),
    'menu_order'            => 0,
    'position'              => 'normal',
    'style'                 => 'default',
    'label_placement'       => 'top',
    'instruction_placement' => 'label',
    'hide_on_screen'        => '',
    'active'                => true,
    'description'           => '',
    'show_in_rest'          => 0,
    'acfe_display_title'    => '',
    'acfe_autosync'         => '',
    'acfe_form'             => 0,
    'acfe_meta'             => '',
    'acfe_note'             => '',
    ));
}
