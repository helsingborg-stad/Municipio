...
export const OrderByControl: React.FC<Props> = ({postType, orderBy, onChange }) => {

    const {postTypeMetaKeys} = useContext(PostsListContext);
    const [metaKeysAsOptions, setMetaKeysAsOptions] = useState<{label: string, value:string}[]>([]);
    
    useEffect(() => {
        postTypeMetaKeys(postType).then((metaKeys) => {
            setMetaKeysAsOptions(metaKeys.map((key) => ({ label: __(`meta: ${key}`, 'municipio'), value: key })));
        });
