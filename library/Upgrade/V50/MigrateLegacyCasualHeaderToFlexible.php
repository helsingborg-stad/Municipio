...
/**
 * Migrates legacy casual header layout to flexible template configuration.
 */
class MigrateLegacyCasualHeaderToFlexible
{
    ...
    private const DESKTOP_UPPER_ITEMS = ['logotype', 'primary'];
    private const RESPONSIVE_UPPER_ITEMS = ['logotype', 'drawer'];
    ...
    {
        if ((string) $this->wpService->getThemeMod(self::HEADER_APPEARANCE_SETTING, '') !== 'casual') {
            return;
        }
    ...
    /**
     * Apply the casual flexible template configuration.
     */
    public function applyTemplate(): void
    {
        $this->wpService->setThemeMod(self::HEADER_APPEARANCE_SETTING, 'flexible');
        $this->wpService->setThemeMod(self::UPPER_SECTION_SETTING, self::DESKTOP_UPPER_ITEMS);
        ...
        $menuAlignments = [
            'casual-left' => 'left',
            'casual-center' => 'center',
            'casual-right' => 'right',
        ];

        if (isset($menuAlignments[$legacyAlignment])) {
            $storage[self::UPPER_SECTION_SETTING]['primary']['align'] = $menuAlignments[$legacyAlignment];
        }
